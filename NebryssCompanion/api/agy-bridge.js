...
function createAgentSession({ onEvent, onError, onClose }) {
  ...
  function buildSystemPreamble(campaignId) {
    ...
    return [
      ...
      '   Available Commands:',
      '   - Context & Lookup: help, get-context, list, get-latest, get-entity, list-entities, list-weapons, calculate-pr, clean-text, auto-tag',
      ...
      `17. MANDATORY FULL ENTITY PARAMETERS ON ALL UPDATES (FETCH BEFORE UPDATE RULE): When updating ANY entity (update-player, update-npc, update-location, update-shop, update-bestiary, update-letter, update-item, update-weapon, update-weapon-rule, update-altered-state, update-affliction), you MUST ALWAYS supply the complete entity parameters in the CLI command. NEVER omit existing entity fields (e.g. for update-player, you must ALWAYS include --campaignId, --id, --name, --race, --origin, --attributes, --weapons, --abilities, --items, --gold, --digitalGold, --talentPoints, --talents, --afflictions, --notes). If the entity's complete attributes, abilities, or items are not fully known in your current context, you MUST FIRST run \`node scripts/campaign-session-tool.js get-entity <type> <id> --campaignId=${targetCamp}\` (which runs automatically in the background) to fetch the complete entity document, merge your modifications into the complete document, and then stage the update command with all fields fully populated.`
    ].filter(Boolean).join('\n');
  }
