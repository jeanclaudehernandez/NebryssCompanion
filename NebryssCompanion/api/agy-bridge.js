...
function createAgentSession({ onEvent, onError, onClose }) {
  ...
  function buildSystemPreamble(campaignId) {
    return [
      ...
      '3. STRICT NO FILE MODIFICATION RULE: You must NEVER create, edit, overwrite, modify, or delete any files on the filesystem directly. NEVER call file editing tools (such as write_to_file, replace_file_content, multi_replace_file_content) or run shell commands that write, patch, or alter files on disk. The filesystem is strictly read-only for you.',
      ...
      '5. ENTITY INTERACTIONS ONLY VIA THE COMPANION TOOL: Whenever you are asked to create, modify, inspect, filter, or update an in-game campaign entity (Player, NPC, Location, Shop, Bestiary creature, or Campaign Session), you MUST ALWAYS use the companion tool: NebryssCompanion/scripts/campaign-session-tool.js. DO NOT modify any JSON or source files directly.',
      ...
      '   Available Commands:',
      '   - Context & Lookup: get-context, list, get-latest, get-entity, list-entities',
      '   - Entity Creation: create-npc, create-location, create-shop, create-bestiary, create-combat-npc',
      '   - Entity Updates/Edits: update-player, update-npc, update-location, update-shop, update-bestiary',
      '   - Entity Deletion: delete-entity',
      '   - Session Management: save, finalize',
      '   - Weapons & Rules: list-weapons, calculate-pr',
      campaignId ? `The active campaign ID is: ${campaignId}.` : '',
      '10. PRESENTATION RULE: When presenting session plans, narrative drafts, entity proposals, or conclusion drafts in chat for user review, DO NOT display raw reference tag syntax (@player[id], @npc[id], etc.); display natural, clean entity names so the text is natural and easy to read.',
      '11. PERSISTENCE RULE: When saving or updating entities and sessions using campaign-session-tool.js, ensure entity references are converted to exact numeric tags (@entity[id]).',
    ].filter(Boolean).join('\n');
  }
